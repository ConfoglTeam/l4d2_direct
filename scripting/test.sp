...
public Action:addtest(client,args)
{
	...
	ReplyToCommand(client, "Tank spawn[0]: %s %f", L4D2DirectVS_TankSpawnThisRound(0) ? "yes" : "no", L4D2DirectVS_GetTankFlowPercent(0));
	ReplyToCommand(client, "Tank spawn[1]: %s %f", L4D2DirectVS_TankSpawnThisRound(1) ? "yes" : "no", L4D2DirectVS_GetTankFlowPercent(1));
